...
- In the triangle ABC the median AD is divided into three equal segments: AE, EF and FD. $$\displaylines{\overrightarrow{BA} \cdot \overrightarrow{CA}=4\\
   \overrightarrow{BF}\cdot\overrightarrow{CF}=-1}$$Find $\overrightarrow{BE}\cdot \overrightarrow{CE}$. $sum$